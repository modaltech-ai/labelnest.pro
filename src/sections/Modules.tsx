    ...
    <article className="card card-hover grid gap-8 p-8 lg:col-span-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-center lg:gap-14 lg:p-10">
      <div>
        <div className="flex items-baseline justify-between gap-4">
          ...
          <span className="t-code shrink-0 text-fg-faint">{pick(m.spec)}</span>
        </div>
        <p className="muted mt-3">{pick(m.body)}</p>
      </div>

      <div>
        <p className="t-code text-fg-faint">{m.detailLabel && pick(m.detailLabel)}</p>
      ...
      <div className="shell">
        <div className="max-w-[46rem]">
          ...
          <p className="t-lead mt-5 max-w-none">
            {voice === 'plain'
              ? 'Everything is joined up. A release knows its artists, its to-do list, its promo, its campaign and its files — so you find things without hunting, and nothing quietly goes missing.'
              : 'Every module links to every other one. A release knows its artists, its tasks, its promo package, its campaign and its files — so finding everything is the default and losing nothing is automatic.'}
          </p>
        </div>
          ...
          {MODULES.map((m) =>
            m.details ? (
              ...
              <article key={m.name} className="card card-hover p-7">
                ...
                <p className="muted mt-3 text-[0.95rem]">{pick(m.body)}</p>
              </article>
            ),
          )}
        </div>
